...
(function() {
   ...
   *
   */
  module.directive('gaDraw', function($translate, $rootScope, $timeout, $q,
      gaBrowserSniffer, gaDefinePropertiesForLayer, gaDebounce, gaFileStorage,
      gaLayerFilters, gaExportKml, gaMapUtils, gaPermalink, $document,
      gaMeasure, gaStyleFactory, gaGeomUtils, gf3Webdav) {
      ...
      link: function(scope, element, attrs, controller) {
        ...
        scope.webdav.load = function() {
          if (scope.webdav.url) {
            ...
            var def = $q.defer();
            gf3Webdav.load(def, scope.map, scope.webdav.url,
              scope.webdav.file, scope.webdav.user, scope.webdav.password);
